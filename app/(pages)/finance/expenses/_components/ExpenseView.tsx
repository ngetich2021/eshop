...
export default function ExpenseView({ stats, expenses, shops, staffList }: Props) {
  ...
  const openModal = (m: "add" | "edit" | "view", e?: Expense) => {
    setMode(m); setSelected(e); setShowForm(true); setOpenDropdownId(null);
  };
  ...
  const filtered = expenses.filter((e) =>
    `${e.description} ${e.category} ${e.shop}`.toLowerCase().includes(search.toLowerCase())
  );

  return (
    <div className="min-h-screen bg-gray-50/80 px-4 py-6 md:px-6">
      <div className="mx-auto max-w-screen-2xl space-y-6">
        {/* STATS GRID — matches PDF: today/week/month/year/total */}
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="relative flex-1 max-w-md">
            ...
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search expenses..." className="w-full rounded-lg border border-gray-300 pl-10 py-2.5 text-sm focus:border-blue-500" />
          </div>
        ...
        <div className="overflow-x-auto rounded-xl border bg-white shadow">
          <table className="w-full min-w-[800px] text-sm">
            <thead className="bg-gray-50">
              <tr>
                {["S/NO","amount","description","date","spentBy","shop","actions"].map((h) => (
            ...
            <tbody className="divide-y divide-gray-100">
              {filtered.map((e, i) => (
                <tr key={e.id} onClick={() => openModal("view", e)} className="cursor-pointer hover:bg-gray-50 transition-all">
                  <td className="px-6 py-4">{i + 1}</td>
                  <td className="px-6 py-4 font-medium">KSh {e.amount.toLocaleString()}</td>
                  <td className="px-6 py-4">{e.description}</td>
                  <td className="px-6 py-4">{e.date}</td>
                  <td className="px-6 py-4">{staffList.find(s => s.id === e.paidById)?.fullName ?? "—"}</td>
                  <td className="px-6 py-4">{e.shop}</td>
                  <td className="px-6 py-4 text-center" onClick={(ev) => ev.stopPropagation()}>
                    <button onClick={(ev) => toggleDropdown(e.id, ev)} className="p-2 hover:bg-gray-100 rounded-full"><MoreVertical size={20} /></button>
                    {openDropdownId === e.id && (
                      <div className="fixed z-[10000] w-40 bg-white border rounded-xl shadow-xl py-1" style={{ top: `${dropdownTop}px`, left: `${dropdownLeft}px` }}>
                        <button onClick={() => { setOpenDropdownId(null); openModal("view", e); }} className="block w-full text-left px-4 py-2 text-sm hover:bg-gray-100">👁️ View</button>
                        <button onClick={() => { setOpenDropdownId(null); openModal("edit", e); }} className="block w-full text-left px-4 py-2 text-sm hover:bg-gray-100">✏️ Edit</button>
                        <button onClick={() => handleDelete(e.id)} disabled={deletingId === e.id} className="block w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-red-50 flex items-center gap-2">
              ...
              {filtered.length === 0 && <tr><td colSpan={7} className="py-20 text-center text-gray-500">No expenses found</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
